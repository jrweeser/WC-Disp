...
class StatBar(ctk.CTkFrame):
    ...
    def __init__(self, master, label: str, **kwargs) -> None:
        ...
        self._label = ctk.CTkLabel(self, text=label, font=theme.font(theme.LIVE_STATS_LABEL))
        self._label.pack(pady=(0, 4))

        row = ctk.CTkFrame(self, fg_color="transparent")
        row.pack(fill="x")

        self._home_val = ctk.CTkLabel(
            row, text="0", width=44, font=theme.font(theme.LIVE_STATS_LABEL, "bold")
        )
        self._home_val.pack(side="left")

        bar_frame = ctk.CTkFrame(row, fg_color="#2b2b2b", corner_radius=6, height=16)
        bar_frame.pack(side="left", fill="x", expand=True, padx=8)
        bar_frame.pack_propagate(False)

        self._home_bar = ctk.CTkFrame(bar_frame, fg_color="#1f6aa5", corner_radius=6, height=16)
        self._home_bar.place(relx=0, rely=0, relwidth=0.5, relheight=1)

        self._away_bar = ctk.CTkFrame(bar_frame, fg_color="#c0392b", corner_radius=6, height=16)
        self._away_bar.place(relx=0.5, rely=0, relwidth=0.5, relheight=1)

        self._away_val = ctk.CTkLabel(
            row, text="0", width=44, font=theme.font(theme.LIVE_STATS_LABEL, "bold")
        )
